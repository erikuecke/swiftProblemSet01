...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    ...
    let boolBugs = (bugs < 10)
    let boolHasMusic = music
    let boolNumOfLevels = (levels > 5)
    var criteriaStatement = "The following criteria must be met for the game release:"
    
    
    if boolBugs, boolHasMusic, boolNumOfLevels {
        release()
    } else {
        if boolBugs == false {
            criteriaStatement = criteriaStatement + " The game must have less than 10 bugs."
        }
        if boolHasMusic == false {
            criteriaStatement = criteriaStatement + " The game needs music."
        }
        if boolNumOfLevels == false {
            criteriaStatement = criteriaStatement + " The game should have more than 5 levels"
        }
        
        print(criteriaStatement)
    }
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if canFinishBike, canFinishRun {
        ...
        print("\(name) is not ready for the duathlon. She can't finish the Bike portion and Run portion.")
    } else if canFinishBike == false && canFinishRun {
        print("\(name) is not ready for the duathlon. She can't finish the Bike portion.")
    }  else if canFinishBike && canFinishRun == false {
        print("\(name) is not ready for the duathlon. She can't finish the Run portion.")
    }
